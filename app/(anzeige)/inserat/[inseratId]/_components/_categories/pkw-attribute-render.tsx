...
const PkwAttributeRender: React.FC<PkwAttributeRenderProps> = ({
    ...
    return (
        <div className="grid grid-cols-2 gap-4 mt-4 p-4 rounded-lg bg-[#13151C] shadow-md">
            ...
            {attributes?.fuel && (
                <div className="flex items-center p-4 bg-[#1a1d25] rounded-lg shadow-sm transition hover:bg-[#2a2d35]">
                    <RiGasStationLine className="w-4 h-4 text-pink-400 mr-2" />
            ...
            {attributes?.doors && (
                <div className="flex items-center p-4 bg-[#1a1d25] rounded-lg shadow-sm transition hover:bg-[#2a2d35]">
                    <DoorClosedIcon className="w-4 h-4 text-red-400 mr-2" />
                    <span className="text-gray-100 font-medium">{attributes.doors}/{Number(attributes.doors) + 1} Türer</span>
                </div>
            )}
            {attributes?.transmission && (
                <div className="flex items-center p-4 bg-[#1a1d25] rounded-lg shadow-sm transition hover:bg-[#2a2d35]">
                   <div> <GearIcon className="w-4 h-4 text-green-400 mr-2" /> </div>
            ...
            {attributes?.ahk && (
                <div className="flex items-center p-4 bg-[#1a1d25] rounded-lg shadow-sm transition hover:bg-[#2a2d35]">
                    <div><Check className="w-4 h-4 text-cyan-400 mr-2" /> </div>
                    <span className="text-gray-100 font-medium w-full break-all">Anhängerkupplung</span>
                </div>
            )}
            {attributes?.loading_volume && (
                <div className="flex items-center p-4 bg-[#1a1d25] rounded-lg shadow-sm transition hover:bg-[#2a2d35]">
                    <HiCubeTransparent className="w-4 h-4 text-orange-400 mr-2" />
