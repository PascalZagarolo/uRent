...
interface MietenCityAnhaengerProps {
  params: {
    city: string;
...
function slugifyCity(str: string) {
  ...
  const MietenCityAnhaengerPage = ({ params }: MietenCityAnhaengerProps) => {
  const { city } = params;
  const category = "anhaenger";
  ...
  const structuredData = {
    ...
    "@type": "CarRental",
    "name": `${categoryLabel} mieten in ${convertVowel(cityObj.name)}`,
  ...
  return (
    <div className="bg-gradient-to-b from-[#14151b] to-[#1a1c25] min-h-screen">
      ...
      <Script id="structured-data-trailer" type="application/ld+json" strategy="afterInteractive">
...
export default MietenCityAnhaengerPage;

export async function generateMetadata({ params }: MietenCityAnhaengerProps) {
