...
interface SelectEmailCreationProps {
  thisInserat: typeof inserat.$inferSelect | any;
currentValue : string;
setCurrentValue : (value) => void;
}

const SelectEmailCreation: React.FC<SelectEmailCreationProps> = ({
  thisInserat,
    currentValue,
    setCurrentValue
}) => {
  ...
  const onPrefill = (usedMail: string) => {
    setCurrentValue(usedMail);
    setModalOpen(false);
  }

  const isAlreadyChosen = (mail: string) => {
    return mail === currentValue;
  }
      ...
      <div className="flex mt-4 ">
        ...
        <div className="  items-center  w-full">
          <Label className="flex justify-start items-center">
            <p className=" font-semibold"> Email-Addresse </p>
          </Label>
          ...
          dark:focus-visible:ring-0 w-full mt-2"
              disabled={isPrefill}
              value={currentValue}
            ...
            >
              <PopoverTrigger asChild>
                <Button className="mt-2 bg-[#0F0F0F] hover:bg-[#0F0F0F] w-1/12 py-5 rounded-l-none" size="sm">
...
export default SelectEmailCreation;