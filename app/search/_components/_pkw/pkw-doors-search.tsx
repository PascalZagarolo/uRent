...
const PkwDoorsSearch = () => {
    ...
    const [currentDoors, setCurrentDoors] = useState(currentObject["doors"]);
    const [currentMaxDoors, setCurrentMaxDoors] = useState(currentObject["doorsMax"]);
    ...
    useEffect(() => {
        if (currentDoors && currentMaxDoors) {
            if (Number(currentDoors) > Number(currentMaxDoors)) {
                changeSearchParams("doors", currentMaxDoors);
